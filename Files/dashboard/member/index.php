...
<?php endif; ?>
    ...
    <div class="page-container sidebar-collapsed" id="navbarcollapse">
        ...
        <div class="main-content">
			...
			<style>#sf-quote-text.sf-fade-out,#sf-quote-sub.sf-fade-out{opacity:0!important;}</style>
			<script>
			var sfQuotes=[
			...
			var sfIdx=0,sfTimer;
			function sfShowQuote(i){var t=document.getElementById('sf-quote-text'),s=document.getElementById('sf-quote-sub');if(!t||!s)return;t.classList.add('sf-fade-out');s.classList.add('sf-fade-out');setTimeout(function(){t.innerText=sfQuotes[i].text;s.innerText=sfQuotes[i].sub;t.classList.remove('sf-fade-out');s.classList.remove('sf-fade-out');},500);}
			function sfNextQuote(){clearInterval(sfTimer);sfIdx=(sfIdx+1)%sfQuotes.length;sfShowQuote(sfIdx);sfTimer=setInterval(function(){sfIdx=(sfIdx+1)%sfQuotes.length;sfShowQuote(sfIdx);},5000);}
					...
					<div class="portal-card" style="background: var(--glass-bg); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid var(--glass-border); border-radius: 20px; padding: 25px; box-shadow: var(--glass-shadow); display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px; color: #ffffff;">
						...
						<div style="flex: 1; border-left: 1px dashed rgba(0,240,255,0.2); padding-left: 25px; min-width: 280px; display: flex; flex-direction: column; justify-content: center;">
							...
							<div style="width: 100%; background: rgba(0,240,255,0.1); height: 8px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 2px 5px rgba(0,0,0,0.5); border: 1px solid rgba(0,240,255,0.3);">
								<div style="width: <?php echo $progress_percent; ?>%; height: 100%; background: linear-gradient(90deg, #00f0ff, #ffd700); border-radius: 4px; box-shadow: 0 0 15px #00f0ff; transition: width 1s ease-in-out;"></div>
							</div>
